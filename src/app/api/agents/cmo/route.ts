/** CMO Agent API Route — daily oversight of captacao + conteudo. */
import { logAgentRun, runAgent } from "@/lib/agents";
import { CMO_AGENT_CONFIG } from "@/lib/agents/configs/cmo";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const maxDuration = 90;

const DEFAULT_TASK = `Execute uma rodada de oversight de growth.
1. list_subordinates
2. get_recent_agent_comms (24h)
3. get_recent_agent_runs (24h)
4. get_pipeline_snapshot
5. Para problemas em captacao/conteudo: dispatch_task; CAC critico: escalate para CEO
...
export async function GET(request: Request): Promise<Response> {
  ...
  const result = await runAgent(CMO_AGENT_CONFIG, DEFAULT_TASK);
...
export async function POST(request: Request): Promise<Response> {
  ...
  const result = await runAgent(CMO_AGENT_CONFIG, task, context);
  logAgentRun(result).catch(() => {});
  return Response.json(
    {
      runId: result.runId,
